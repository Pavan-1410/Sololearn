...
function DynamicQuizPage({ lesson, onBack, onComplete }) {
  ...
  // Safety check
  if (!lesson || !lesson.quizQuestions || lesson.quizQuestions.length === 0) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 flex items-center justify-center p-4">
        <div className="text-center p-12 bg-white/10 backdrop-blur-xl rounded-3xl border border-white/20 shadow-2xl">
          ...
          <p className="text-red-300 text-lg mb-6">Error: No quiz questions available for this lesson.</p>
          <button 
            onClick={onBack} 
            className="px-8 py-3 bg-white/20 hover:bg-white/30 text-white rounded-xl border border-white/30 transition-all duration-300 hover:scale-105 backdrop-blur-sm"
          >
            ← Back to Lesson
          </button>
        </div>
  ...
  const progress = ((currentQuestionIndex + 1) / lesson.quizQuestions.length) * 100;
  ...
  const handleNext = () => {
    if (currentQuestionIndex < lesson.quizQuestions.length - 1) {
      ...
      <div className="relative z-10 max-w-4xl mx-auto p-6 min-h-screen flex flex-col">
        ...
        <div className="mb-8 text-center">
          ...
          <div className="flex items-center justify-center space-x-4 text-white/80">
            <span className="text-lg">Question {currentQuestionIndex + 1} of {lesson.quizQuestions.length}</span>
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="w-full max-w-2xl bg-white/10 backdrop-blur-xl rounded-3xl border border-white/20 shadow-2xl p-8 transform transition-all duration-500 hover:scale-[1.02]">
            {!currentQuestion ? (
              <p className="text-red-300 text-center text-lg">Error: No more questions available.</p>
            ) : (
              <>
                {/* Question */}
                <div className="mb-8">
                ...
                <div className="flex justify-between items-center pt-6 border-t border-white/20">
                  ...
                  >
                    {currentQuestionIndex < lesson.quizQuestions.length - 1 ? "Next Question →" : "Finish Quiz 🏁"}
                  </button>
                </div>
              </>
            )}
